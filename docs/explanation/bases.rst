...
Snaps declare a base in their project file. The base defines the feature set used by
Snapcraft, the ``snapcraft.yaml`` project file, the environment where the snap is built,
...
:external+core:doc:`index` systems need a base snap. These base snaps must be bootable
and include ``systemd``.
