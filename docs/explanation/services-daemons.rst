...
Use the ``apps`` key in the project file to select a daemon model. The example below shows how to define a simple daemon by using the ``daemon`` key:
...
For the complete list of app daemon keys and supported values, see :ref:`App daemon key reference <reference-snapcraft-yaml-app-keys-app-daemon>`.
