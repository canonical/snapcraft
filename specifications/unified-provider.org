...
class ProviderV2:
  ...
  def is_provider_ready(self) -> bool:
    ...
    def setup_provider(self, interactive: bool) -> None:
      ...
      :raises ProviderSetupError: if provider has dependencies that need to be
      installed, or configuration that must be changed.
      """
...
- =snapcraft <step>= is not compatible with Lauchpad.
...
='<command>' is not available for the currently configured provider
'<provider>'.=

For =--debug=, =--shell=, and =--shell-after= options:
='<command>' does not support option '<option>' for the currently configured
provider.=
