...
class CleanTestCase(integration_tests.TestCase):
    ...
    def test_clean_invalid_yaml(self):
        ...
        snapdir = os.path.join(project_dir, 'snap')
        self.run_snapcraft('clean', project_dir)
        self.assertThat(os.path.join(snapdir, 'parts'), Not(DirExists()))
        self.assertThat(os.path.join(snapdir, 'stage'), Not(DirExists()))
        self.assertThat(os.path.join(snapdir, 'prime'), Not(DirExists()))
