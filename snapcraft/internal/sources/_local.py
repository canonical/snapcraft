...
class Local(Base):
    ...
    def __init__(self, *args, copy_function=file_utils.link_or_copy, **kwargs):
        ...
        self._ignore = functools.partial(
            _ignore, self.source_abspath, os.getcwd())

    def pull(self):
        file_utils.link_or_copy_tree(
            self.source_abspath, self.source_dir, ignore=self._ignore,
    ...
    def _check(self, target):
        ...
        for (root, directories, files) in os.walk(self.source_abspath,
                                                  topdown=True):
        ...
        return (
            len(self._updated_files) > 0 or
            len(self._updated_directories) > 0)
