...
class HelpCommandTestCase(tests.TestCase):
    ...
    def test_topic_and_plugin_not_found_exists_with_tip(self):
    ...
    def test_print_module_help_for_valid_plugin(self, mock_stdout):
        ...
        self.assertTrue(output.startswith('The nil plugin is'),
                        'Help starts with {!r}'.format(output))

    @mock.patch.object(builtins, 'help')
    def test_show_module_help_with_devel_for_valid_plugin(self, mock_help):
        help.main(['nil', '--devel'])

        module = importlib.import_module('snapcraft.plugins.nil')
        mock_help.assert_called_once_with(module)
    ...
    def test_print_topics(self, mock_stdout):
        ...
        for t in help._TOPICS:
            self.assertTrue(
                t in output,
                'Missing topic: {!r} in {}'.format(t, output))
    ...
    def test_print_topic_help_for_valid_topic(self, mock_stdout):
        ...
        output = mock_stdout.getvalue()
        self.assertTrue(output.startswith('Common keywords for plugins'),
                        'Help starts with {!r}'.format(output))

    @mock.patch.object(builtins, 'help')
    def test_print_topic_help_with_devel_for_valid_topic(self, mock_help):

        for key in help._TOPICS:
            mock_help.reset_mock()
            with self.subTest(key=key):
                help.main([key, '--devel'])
                mock_help.assert_called_once_with(help._TOPICS[key])
