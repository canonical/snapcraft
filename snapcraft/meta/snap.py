...
from snapcraft.meta import errors
from snapcraft.meta.application import Application
from snapcraft.meta.hooks import Hook
from snapcraft.meta.package_repository import PackageRepository
from snapcraft.meta.plugs import ContentPlug, Plug
from snapcraft.meta.slots import ContentSlot, Slot
from snapcraft.meta.system_user import SystemUser

logger = logging.getLogger(__name__)


def get_installed_snap_path(snap_name: str):
    """Return the path to the currently installed snap."""
...
class Snap:
    ...
    def get_provider_content_directories(self) -> Set[str]:
        ...
        for plug in self.get_content_plugs():
            ...
            provider_path = get_installed_snap_path(provider)
            ...
            for slot in snap.get_content_slots():
                slot_installed_path = get_installed_snap_path(provider)
    ...
    def from_dict(cls, snap_dict: Dict[str, Any]) -> "Snap":  # noqa: C901
        """Create a Snap objects getting the data from a loaded project."""
