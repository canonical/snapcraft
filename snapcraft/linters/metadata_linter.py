...
_FIELDS: list[MetadataField] = [
    ...
    # Rank 1 fields
    MetadataField("title", LinterResult.WARNING, lambda meta: meta.title),
    MetadataField(
        "contact", LinterResult.WARNING, lambda meta: _get_links_attr(meta, "contact")
    ),
    MetadataField("license", LinterResult.WARNING, lambda meta: meta.license),
    MetadataField(
        "common_id",
    ...
    # Rank 2 fields
    MetadataField(
        "donation", LinterResult.INFO, lambda meta: _get_links_attr(meta, "donation")
    ),
    MetadataField(
        "issues", LinterResult.INFO, lambda meta: _get_links_attr(meta, "issues")
    ),
    MetadataField(
        "source_code",
        LinterResult.INFO,
    ...
    MetadataField(
        "website", LinterResult.INFO, lambda meta: _get_links_attr(meta, "website")
    ),
]
...
class MetadataLinter(Linter):
    ...
    def run(self) -> list[LinterIssue]:
        ...
        for field in _FIELDS:
            if field.name == "common_id" and (meta.type and meta.type != "app"):
                continue
