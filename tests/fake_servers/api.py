...
class FakeStoreAPIServer(base.BaseFakeServer):
    ...
    def configure(self, configurator):
        ...
        configurator.add_route(
            "snap_channel_map",
            urllib.parse.urljoin(self._DEV_API_PATH, "{snap}/channel-map"),
            request_method="GET",
        )
    ...
    def snap_channel_map(self, request):
        ...
        snap_channel_map = {
            ...
            "revisions": [
                {"architectures": ["amd64", "arm64", "armhf", "i386", "s390x", "ppc64el"], "revision": 19, "version": "10"}
            ],
            "snap": {
                "channels": [
                    {
        ...
        content_type = "application/json"
        return response.Response(
            json.dumps(snap_channel_map).encode(), response_code, [("Content-Type", content_type)]
        )
