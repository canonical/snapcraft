...
def test_expand_environment(new_dir, mocker):
    ...
    yaml_data = {
        ...
        "field2": "$CRAFT_PROJECT_VERSION",
        "field3": "$SNAPCRAFT_PROJECT_VERSION",
        "field4": "$CRAFT_PROJECT_GRADE",
        "field5": "$SNAPCRAFT_PROJECT_GRADE",
        "field6": "$CRAFT_ARCH_TRIPLET",
        "field7": "$SNAPCRAFT_ARCH_TRIPLET",
        "field8": "$CRAFT_TARGET_ARCH",
        "field10": "$SNAPCRAFT_TARGET_ARCH",
        "dirs": {
            "field11": ["$CRAFT_STAGE", "$SNAPCRAFT_STAGE"],
            "field12": ["${CRAFT_PRIME}", "${SNAPCRAFT_PRIME}"],
            "field13": ["$CRAFT_PROJECT_DIR", "$SNAPCRAFT_PROJECT_DIR"],
        },
        "field14": ["$CRAFT_PARALLEL_BUILD_COUNT", "$SNAPCRAFT_PARALLEL_BUILD_COUNT"],
    }
    ...
    assert yaml_data == {
        ...
        "field4": "stable",
        "field5": "stable",
        "field6": "aarch64-linux-gnu",
        "field7": "aarch64-linux-gnu",
        "field8": "arm64",
        "field10": "arm64",
        "dirs": {
            "field11": [f"{new_dir}/stage", f"{new_dir}/stage"],
            "field12": [f"{new_dir}/prime", f"{new_dir}/prime"],
            "field13": [f"{new_dir}", f"{new_dir}"],
        },
        "field14": ["8", "8"],
    }
