...
class QbsPluginTestCase(tests.TestCase):
    ...
    def test_schema(self):
        ...
        self.assertTrue(
            "qbs-options" in properties, 'Expected "options" to be in properties'
        )
        self.assertTrue(
            "qt-version" in properties, 'Expected "qt-version" to be in properties'
        )
        self.assertTrue(
            "qbs-profile" in properties, 'Expected "profile" to be in properties'
        )
        self.assertTrue(
            "qbs-build-variant" in properties,
        ...
        self.assertTrue(
            "enum" in profile, 'Expected "enum" to be included in "profile"'
        )
        self.assertTrue(
            "default" in profile, 'Expected "default" to be included in "profile"'
        )
        ...
        self.assertTrue(
            "enum" in build_variant, 'Expected "enum" to be included in "profile"'
        )
        self.assertTrue(
            "default" in build_variant, 'Expected "default" to be included in "profile"'
        )
    ...
    def test_unsupported_qt_version(self):
        ...
        raised = self.assertRaises(
            RuntimeError, qbs.QbsPlugin, "test-part", self.options, self.project_options
        )

        self.assertEqual(str(raised), "Unsupported Qt version: 'qt3'")

    @mock.patch.object(qbs.QbsPlugin, "run")
    def test_build_nil(self, run_mock):
        self.options.qt_version = None
        plugin = qbs.QbsPlugin("test-part", self.options, self.project_options)
    ...
    def test_build_qt(self, run_mock):
        ...
        run_mock.assert_has_calls(
            [
                mock.call(["qbs", "setup-toolchains", "--detect"], env=mock.ANY),
                mock.call(
                    ["qbs", "setup-qt", qmake_path, "snapcraft-qbs-qt5-clang"],
                    env=mock.ANY,
                ),
                mock.call(
                    [
                ...
                mock.call(
                    [
                        ...
                        "release",
                        "qbs.installRoot:" + plugin.installdir,
                        "profile:snapcraft-qbs-qt5-clang",
                    ],
                    env=mock.ANY,
                ),
            ]
