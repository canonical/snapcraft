...
class TestLinkOrCopyPathList(unit.TestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_link_or_copy_path_list_all_files(self):
        os.makedirs("dst")

        path_list = ["1", os.path.join("foo", "2"), os.path.join("foo", "bar", "3")]

        file_utils.link_or_copy_path_list(
            source_tree="src", destination_tree="dst", path_list=path_list
        )
    ...
    def test_link_or_copy_path_list_dir(self):
        os.makedirs("dst")

        path_list = ["1", os.path.join("foo")]
