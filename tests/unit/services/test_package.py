...
def test_materialize_extra_assets_replaces_placeholder_meta_hook(
    ...
    placeholder_hook.write_text(
        "#!/bin/true\n# Generated by Snapcraft as a placeholder hook.\n"
    )
...
def test_materialize_extra_assets_preserves_unmarked_noop_meta_hook(
    default_project, fake_services, setup_project
):
    setup_project(fake_services, default_project.marshal(), write_project=True)
    package_service = cast(Package, fake_services.get("package"))
    ...
    existing_hook = prime_dir / "meta" / "hooks" / "configure"
    existing_hook.parent.mkdir(parents=True)
    existing_hook.write_text("#!/bin/true\n")
    existing_hook.chmod(0o755)

    package_service._materialize_extra_assets(None)

    assert existing_hook.read_text() == "#!/bin/true\n"
    assert oct(existing_hook.stat().st_mode)[-3:] == "755"
