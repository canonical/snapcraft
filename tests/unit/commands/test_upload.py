...
def test_default_channels(
    emitter, fake_store_notify_upload, fake_store_verify_upload, snap_file, app_config
):
    cmd = commands.StoreUploadCommand(app_config)
...
def test_invalid_file(app_config):
    cmd = commands.StoreUploadCommand(app_config)
