...
def test_release(emitter, fake_store_release, app_config):
    cmd = commands.StoreReleaseCommand(app_config)
...
@pytest.mark.usefixtures("memory_keyring")
def test_release_multiple_channels(emitter, fake_store_release, app_config):
    cmd = commands.StoreReleaseCommand(app_config)
...
@pytest.mark.usefixtures("memory_keyring")
def test_release_progressive(emitter, fake_store_release, app_config):
    cmd = commands.StoreReleaseCommand(app_config)
...
@pytest.mark.usefixtures("memory_keyring")
def test_close(emitter, fake_store_close, app_config):
    cmd = commands.StoreCloseCommand(app_config)
...
def test_close_no_snap_id(emitter, app_config):
    cmd = commands.StoreCloseCommand(app_config)
