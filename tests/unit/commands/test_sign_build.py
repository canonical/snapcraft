...
class SignBuildTestCase(FakeStoreCommandsBaseTestCase):
    ...
    def test_sign_build_no_usable_keys(
        ...
        self.useFixture(
            fixtures.MockPatch("subprocess.check_output", return_value=b"[]")
        )
    ...
    def test_sign_build_no_usable_named_key(
        ...
        self.useFixture(
            fixtures.MockPatch(
                "subprocess.check_output", return_value=b'[{"name": "default"}]'
            )
        )
    ...
    def test_sign_build_unregistered_key(
        ...
        self.useFixture(
            fixtures.MockPatch(
                ...
                return_value=b'[{"name": "default", "sha3-384": "a_hash"}]',
            )
        )
    ...
    def test_sign_build_upload_successfully(
        ...
        self.assertThat(
            result.output,
            Contains(
                f"Build assertion {snap_build_path} uploaded to the Store."
            ),
        )
        self.assertThat(snap_build_path, FileExists())
        fake_check_output.mock.assert_called_with(
