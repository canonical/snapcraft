...
class CollaboratorsCommandTestCase(CollaborateBaseTestCase):
    ...
    def test_collaborators_update_developers(self):
        result = self.run_command([self.command_name, 'test-snap-with-dev',
                                   '--key-name', 'key-name'])

        self.assertThat(result.exit_code, Equals(0))
        self.popen_mock.assert_called_with(['snap', 'sign', '-k', 'key-name'],
    ...
    def test_no_prior_developer_assertion(self):
        result = self.run_command([self.command_name, 'core-no-dev',
                                   '--key-name', 'key-name'])
    ...
    def test_collaborate_no_revoke_uploads_request(self):
        ...
        self.assertThat(result.output, Contains(
            'This will revoke the following collaborators: \'this\''))
        self.assertThat(result.output, Contains(
            'Are you sure you want to continue? [y/N]:'))
        self.assertThat(result.output, Contains(
            'The collaborators for this snap have not been altered.'))
    ...
    def test_collaborate_unchanged_collaborators(self):
        ...
        self.assertThat(result.exit_code, Equals(0))
        self.assertThat(result.output, Contains(
            'Aborting due to unchanged collaborators list.'))
    ...
    def test_collaborate_yes_revoke_uploads_request(self):
        ...
        self.assertThat(str(raised), Contains(
            'Received error 409: "{}"'.format(
                "The assertion's `developers` would revoke "
                "existing uploads.")))
