...
class QemuDriverGeneralTest(QemuDriverBaseTest):
    ...
    def test_multipass_command_missing_raises(self):
        ...
        self.assertRaises(errors.ProviderCommandNotFound,
                          QemuDriver,
                          ssh_key_file='id_rsa', ssh_username='builder')
...
class QemuDriverPassthroughBaseTest(QemuDriverBaseTest):
    ...
    def setUp(self):
        ...
        self.qemu_driver = QemuDriver(ssh_key_file='fake-id_rsa',
...
class QemuDriverLaunchTest(QemuDriverPassthroughBaseTest):
    ...
    def test_launch(self):
        self.qemu_driver.launch(hda='test-vm.qcow2',
                                qcow2_drives=['cloudlocal.qcow2'],
                                project_9p_dev='/snapcraft-project',
                                ram='1')

        self.popen_mock.assert_has_calls([mock.call(), mock.call([
            'sudo', self.qemu_driver.provider_cmd, '-m', '1', '-nographic',
            '-monitor', 'telnet::{},server,nowait'.format(
...
class QemuDriverExecuteTest(QemuDriverPassthroughBaseTest):
    ...
    def test_execute_fails(self):
        ...
        self.assertRaises(errors.ProviderExecError,
                          self.qemu_driver.execute,
                          command=['ls', '/root'])
