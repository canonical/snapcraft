...
_setup_ubuntu_base_chroot_fnc = [
    textwrap.dedent(
        ...
        setup_chroot_base() {
            ...
            # if not running native, prepare root for qemu-static execution
            if [ "${CRAFT_ARCH_BUILD_ON}" != "${CRAFT_ARCH_BUILD_FOR}" ]; then
                cp --no-dereference /usr/lib/"${CRAFT_ARCH_TRIPLET_BUILD_FOR}"/fakechroot/* \\
                                    "${UC_INITRD_ROOT}/usr/lib/${CRAFT_ARCH_TRIPLET_BUILD_FOR}"
                cp --no-dereference /usr/lib/"${CRAFT_ARCH_TRIPLET_BUILD_FOR}"/libfakeroot/* \\
                                    "${UC_INITRD_ROOT}/usr/lib/${CRAFT_ARCH_TRIPLET_BUILD_FOR}"
            fi
...
_chroot_run_cmd_fnc = [
    textwrap.dedent(
        ...
        run_chroot() {
            local chroot_home="${1}"
            local cmd="${2}"
            fakechroot fakeroot chroot "${chroot_home}" /bin/bash -c "${cmd}"
        }
        """
    )
]
...
_setup_initrd_chroot_fnc = [
    textwrap.dedent(
        ...
        # 1: work dir, 2: ubuntu series, 4: ppa fingerprint
        setup_initrd_chroot() {
            set +x
            local work_dir="${1}"
            local series="${2}"
            local ppa_fingerprint="${3}"

            if [ ! -e "${work_dir}/.${UC_INITRD_ROOT_NAME}.base" ]; then
                setup_chroot_base "${work_dir}" "${series}"
            ...
            if [ ! -e "${work_dir}/.${UC_INITRD_ROOT_NAME}.ppa" ]; then
                ...
                run_chroot "${UC_INITRD_ROOT}" "apt-get install --no-install-recommends -y ca-certificates debconf-utils libfakeroot libsystemd-shared lz4 xz-utils zstd"
                chroot_add_snappy_dev_ppa "${work_dir}" "${series}" "${ppa_fingerprint}"
                touch "${work_dir}/.${UC_INITRD_ROOT_NAME}.ppa"
            fi

            if [ ! -e "${work_dir}/.${UC_INITRD_ROOT_NAME}.fix-systemd-shared" ]; then
                if [ "${CRAFT_ARCH_BUILD_ON}" != "${CRAFT_ARCH_BUILD_FOR}" ]; then
                    local lib_path
                    lib_path=$(find "${UC_INITRD_ROOT}/usr/lib" -type f -name 'libsystemd-shared-*.so' -exec realpath {} \\;)
                    lib_file=$(basename "${lib_path}")
                    ln -sf "$(realpath --relative-to "${UC_INITRD_ROOT}/usr/lib/${CRAFT_ARCH_TRIPLET_BUILD_FOR}" "${lib_path}")" \\
                           "${UC_INITRD_ROOT}/usr/lib/${CRAFT_ARCH_TRIPLET_BUILD_FOR}/$(basename "${lib_path}")"
                fi
                touch "${work_dir}/.${UC_INITRD_ROOT_NAME}.fix-systemd-shared"
            fi
...
_setup_initrd_build_env_cmd = [
    textwrap.dedent(
        ...
        setup_initrd_chroot "${{CRAFT_PART_SRC}}" "${{UBUNTU_SERIES}}" "{_SNAPPY_DEV_KEY_FINGERPRINT}"
        """
    )
]
...
_initrd_tool_workaroud_cmd = [
    textwrap.dedent(
        ...
        # actual ubuntu-core initramfs build is performed in fakechroot
        # in fakechroot, tmp is not really tmpfs, avoid excessive use of cp
        # cp "-ar"/"-aR" -> cp "-lR"
