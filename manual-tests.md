...
# Test a different cross-compile toolchain.

1. Find the latest arm Linux cross-toolchain available:

    https://www.kernel.org/pub/tools/crosstool:
    ...
    arm arm-unknown-linux-gnueabi   4.6.3
    ...

2.  Download the tarball and install it in /opt/cross:

    $ wget https://www.kernel.org/pub/tools/crosstool/files/bin/x86_64/4.6.3/x86_64-gcc-4.6.3-nolibc_arm-unknown-linux-gnueabi.tar.gz
    $ sudo mkdir /opt/cross
    $ sudo tar zxvf x86_64-gcc-4.6.3-nolibc_x86_64-linux.tar.gz -C /opt/cross

3. Get the PC kernel source:

    $ git clone -b pc https://git.launchpad.net/~ubuntu-kernel/ubuntu/+source/linux-snap/+git/xenial
    $ cd kernel

4. Adjust the $PATH and run snapcraft:

    $ export PATH=$PATH:/opt/cross/gcc-4.6.3-nolibc/arm-unknown-linux-gnueabi/bin
    $ export CROSS_COMPILE=arm-unknown-linux-gnueabi-
    $ snapcraft --target-arch=armhf

5. CTRL+C after the .config is written and the compilation starts:

    ...
    #
    # configuration written to .config
    #
    make -j4 ARCH=arm CROSS_COMPILE=arm-unknown-linux-gnueabi- bzImage modules dtbs
    scripts/kconfig/conf  --silentoldconfig Kconfig
    ...
    <CTRL+C after this>


  * Check the toolchain picked up is the one you specified in CROSS_COMPILE
    instead of the default one ('arm-unknown-linux-gnueabi-' vs
    'arm-linux-gnueabihf-')
